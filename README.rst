...
Repository to build numexpr wheels.

To update:

* Update numexpr submodule with version you want to build:

    * cd numexpr && git pull && git checkout DESIRED_TAG
    * cd .. && git add numexpr

* Check minimum numpy versions to build against in ``.travis.yml`` file.  You
  need to build against the earliest numpy that numexpr is compatible with; see
