...
* making a commit to the ``numexpr-wheels`` repository (e.g. with ``git commit
  --allow-empty``); or
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx numexpr 2.6.0
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 numexpr 2.6.0
